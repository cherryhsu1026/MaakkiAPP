...
// 資料功能類別
public class PrevNotificationDAO {
    // 表格名稱
    ...
    public static final String CREATE_TABLE =
            "CREATE TABLE " + TABLE_NAME + " (" +
                    ...
                    //isActivity_COLUMN + " INTEGER NOT NULL, " +
                    //ActivityOrUrl_COLUMN + " TEXT NOT NULL, " +
                    LASTMODIFY_COLUMN + " INTEGER NOT NULL)";
    // 資料庫物件
    private SQLiteDatabase db;

    // 建構子，一般的應用都不需要修改
    public PrevNotificationDAO(Context context) {
    ...
    // 新增參數指定的物件
    public PrevNotification insert(PrevNotification prev) {
        ...
        //cv.put(Icon_COLUMN, prev.geticon());
    ...
    // 修改參數指定的物件
    public boolean update(PrevNotification prev) {
    ...
    // 讀取所有記事資料
    public List<PrevNotification> getAll() {
        List<PrevNotification> result = new ArrayList<>();
    ...
    // 取得指定編號的資料物件
    public PrevNotification get(long id) {
        // 準備回傳結果用的物件
        PrevNotification prev = null;
        // 使用編號為查詢條件
    ...
    // 取得指定編號的資料物件
    public PrevNotification getbyNotificationiID(int id) {
        // 準備回傳結果用的物件
        PrevNotification prev = null;
        // 使用編號為查詢條件
    ...
    // 把Cursor目前的資料包裝為物件
    public PrevNotification getRecord(Cursor cursor) {
        // 準備回傳結果用的物件
        PrevNotification result = new PrevNotification();
        result.setid(cursor.getLong(0));
        result.settitle(cursor.getString(1));
        result.setmessage(cursor.getString(2));
        result.setmessagetype(cursor.getInt(3));
        result.setLastModify(cursor.getLong(4));
        //result.seticon(cursor.getInt(4));
        //result.setisActivity(cursor.getInt(5));
        //result.setActivityOrUrl(cursor.getString(6));
        //
