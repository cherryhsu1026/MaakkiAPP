...
public class OrderDetailDAO {
    ...
    public static final String Set_COLUMN = "set_string";
    ...
    public OrderDetail insert(OrderDetail orderdetail) {
        ...
        cv.put(OrderId_COLUMN, orderdetail.getOrderId());
        cv.put(ItemId_COLUMN, orderdetail.getItemId());
        cv.put(Qty_COLUMN, orderdetail.getQty());
        cv.put(Set_COLUMN, orderdetail.getSetString());
        // 提醒日期時間
    ...
    public boolean update(OrderDetail orderdetail) {
        ...
        // 設定修改資料的條件為編號
        // 格式為「欄位名稱＝資料」
        String where = KEY_ID + "=" + orderdetail.getDetailId();
    ...
    public boolean deleteByOrderId(long order_id) {
        // 設定條件為編號，格式為「欄位名稱=資料」
        String where = OrderId_COLUMN + "=" + order_id;
        // 刪除指定編號資料並回傳刪除是否成功
        return db.delete(TABLE_NAME, where, null) > 0;
    }

    public boolean deleteByItemId(long order_id, long id) {
        // 設定條件為編號，格式為「欄位名稱=資料」
        String where = ItemId_COLUMN + "=" + id + " AND " + OrderId_COLUMN + " = " + order_id;
        // 刪除指定編號資料並回傳刪除是否成功
        return db.delete(TABLE_NAME, where, null) > 0;
    }
    ...
    public List<OrderDetail> getOrderDetailByOrderId(long order_id) {
        List<OrderDetail> result = new ArrayList<>();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + OrderId_COLUMN + " = " + order_id, null);
        while (cursor.moveToNext()) {
            result.add(getRecord(cursor));
        }
    ...
    // 把Cursor目前的資料包裝為物件
    public OrderDetail getRecord(Cursor cursor) {
        // 準備回傳結果用的物件
        OrderDetail result = new OrderDetail();
        result.setDetailId(cursor.getLong(0));
        result.setOrderId(cursor.getLong(1));
        result.setItemId(cursor.getLong(2));
        result.setQty(cursor.getInt(3));
        result.setSetString(cursor.getString(4));
