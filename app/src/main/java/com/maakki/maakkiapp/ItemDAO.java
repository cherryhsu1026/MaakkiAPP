...
public class ItemDAO {
    ...
    public static final String CategoryId_COLUMN = "cat_id";
    ...
    public static final String CREATE_TABLE =
            "CREATE TABLE " + TABLE_NAME + " (" +
                    ...
                    ItemName_COLUMN + " TEXT NOT NULL, " +
                    //CategoryId_COLUMN+ " INTEGER NOT NULL)" ;
                    CategoryId_COLUMN + " INTEGER NOT NULL, " +
                    ItemType_COLUMN + " INTEGER NOT NULL, " +
    ...
    public ItemDAO(Context context) {
        ...
        //db = ItemDBHelper.getDatabase(context);
    }
    ...
    public Item insert(Item item) {
        ...
        cv.put(ItemName_COLUMN, item.getItemName());
        cv.put(CategoryId_COLUMN, item.getCatId());
        cv.put(ItemType_COLUMN, item.getItemType());
        cv.put(Price_Column, item.getPrice());
        cv.put(Status_Column, item.getStatus());
        cv.put(Position_Column, item.getPosition());
    ...
    public boolean update(Item item) {
        ...
        // 設定修改資料的條件為編號
        // 格式為「欄位名稱＝資料」
        String where = KEY_ID + "=" + item.getItemId();
    ...
    public List<Item> getActivatedItemByCategory(long cat_id) {
        List<Item> result = new ArrayList<>();
        //Cursor cursor = db.query(
        //        TABLE_NAME, null, null, null, null, null, null, null);
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " ORDER BY " + Position_Column + " ASC ", null);
        while (cursor.moveToNext()) {
            if (getRecord(cursor).getCatId() == cat_id && getRecord(cursor).getStatus() == 0) {
                result.add(getRecord(cursor));
            }
        }
        cursor.close();
    ...
    //已經是刪除的品項就不顯示了
    public List<Item> getDisplayedItemByCategory(long cat_id) {
        List<Item> result = new ArrayList<>();
        //Cursor cursor = db.query(
        //        TABLE_NAME, null, null, null, null, null, null, null);
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_NAME + " ORDER BY " + Position_Column + " ASC ", null);
        while (cursor.moveToNext()) {
            if (getRecord(cursor).getCatId() == cat_id & getRecord(cursor).getStatus() < 2) {
                result.add(getRecord(cursor));
            }
        }
        cursor.close();
    ...
    public Item getRecord(Cursor cursor) {
        ...
        result.setItemId(cursor.getLong(0));
        result.setItemName(cursor.getString(1));
        result.setCatId(cursor.getLong(2));
        result.setItemType(cursor.getInt(3));
        result.setPrice(cursor.getDouble(4));
        result.setStatus(cursor.getInt(5));
        result.setPosition(cursor.getInt(6));
